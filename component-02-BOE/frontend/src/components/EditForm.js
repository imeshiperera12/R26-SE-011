...
function EditForm({ candidateData, module, onSaveSuccess }) {
  ...
  // =====================================================
  // MODULE-LEVEL REVIEW STATE
  // =====================================================

  const isLocked =
    !module || module.status === "LOCKED" || module.finalized === true;

  // =====================================================
  // REVIEW DEADLINE
  // =====================================================

  const reviewDeadline = module?.reviewDeadline
    ? new Date(module.reviewDeadline)
    : null;

  const daysLeft =
    reviewDeadline && !isNaN(reviewDeadline.getTime())
      ? Math.max(
          0,
          Math.ceil((reviewDeadline - new Date()) / (1000 * 60 * 60 * 24)),
        )
      : 0;
  ...
  const handleSubmit = async () => {
    if (isLocked) {
      showMsg(
        "Editing is locked because the module review period has ended.",
        "error",
      );
    ...
    try {
      ...
      // ── Re-fetch candidate to refresh all UI data ──
      onSaveSuccess();
    } catch (error) {
    ...
    <div className="ef-root">
      ...
      {isLocked && (
        <div className="ef-lock-notice">
          ...
          Editing is locked — the module review period has ended.
        </div>
      )}
      ...
      <div className="ef-form">
        ...
        <div className="ef-field">
          <label className="ef-label" htmlFor="ef-marks">
            New Marks
          </label>
        ...
        <div className="ef-field">
          ...
          <div
            className={`ef-grade-preview ef-grade-preview--${gradeClass(
              previewGrade,
            )}`}
          >
            {previewGrade ? (
        ...
        <div className="ef-field ef-field--full">
          <label className="ef-label" htmlFor="ef-reason">
            Correction Reason
          </label>
